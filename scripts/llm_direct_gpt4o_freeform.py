#!/usr/bin/env python3
"""
LLM Direct Validation: Single-stage error detection using GPT-4o (FULL MODEL)

Same setup as original llm_direct_validation.py but with GPT-4o instead of GPT-4o-mini.
This tests whether the model upgrade alone improves performance.
"""
...
def load_marketing_text(run_id):
    """Load marketing text from pilot study directory"""
    # Map run_id to pilot file
    if 'smartphone' in run_id:
        ...
        raise ValueError(f"Unknown product in run_id: {run_id}")

    # Extract number from run_id (e.g., user_smartphone_4 -> 4)
    num = run_id.split('_')[-1]
    file_path = f"pilot_study/{product}/files/{product}_{num}.txt"
...
def validate_with_llm(marketing_text, specs_text, run_id):
    """
    Use GPT-4o (FULL MODEL) to directly identify factual errors by comparing
...
"""
    ...
    try:
        response = client.chat.completions.create(
            model='gpt-4o',  # *** CHANGED: Full GPT-4o instead of gpt-4o-mini ***
            messages=[
...
def main():
    ...
    print("=" * 80)
    print("LLM DIRECT VALIDATION - GPT-4o (FULL) @ temp=0 (FREE-FORM TEXT)")
    print("=" * 80)
    ...
    # Save results
    output_file = "results/llm_direct_gpt4o_freeform_results.csv"
    ...
    # Also save detailed text responses
    text_output_dir = Path("results/llm_direct_gpt4o_freeform_responses")
    text_output_dir.mkdir(exist_ok=True)

    for result in results:
        response_file = text_output_dir / f"{result['run_id']}.txt"
    ...
    print(f"Detailed responses saved to: {text_output_dir}")
    print()
