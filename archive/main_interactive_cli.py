...
# --- 2. PROMPT LOADING FROM NEW SYSTEM ---

def load_prompt_index(index_path: Path = Path("results/prompts_index.csv")):
    """
    Load the prompt index CSV generated by the matrix generator.

    Returns:
        List of prompt dictionaries with run_id, product_id, template, filepath, etc.
    """
    if not index_path.exists():
        print(f"WARNING: Prompt index not found at {index_path}")
        print("Please run: python -m runner.generate_matrix")
        return []
...
def load_prompt_text(filepath: str) -> str:
    """
    Load the actual prompt text from a file.

    Args:
        filepath: Path to the prompt text file
...
def run_new_conversation(session_state, db):
    """
    Handles the workflow for a new conversation using the new prompt system.
    """
    print("\n--- New Conversation ---")
    session_state["conversation_id"] = str(uuid.uuid4())
    session_state["repetition_id"] = 1

    # Load available prompts from the index
    available_prompts = load_prompt_index()
    if not available_prompts:
        print("ERROR: No prompts available. Please run: python -m runner.generate_matrix")
        return
    ...
    # Display prompts with product and template info
    print("\nSelect a Prompt:")
    for i, prompt in enumerate(available_prompts, 1):
        template_name = Path(prompt['template']).stem
        print(f"[{i}] {prompt['run_id'][:8]} | {prompt['product_id']} | {template_name} | trap={prompt['trap_flag']}")

    while True:
        try:
            choice = int(input("Enter number: "))
            if 1 <= choice <= len(available_prompts):
                selected_prompt = available_prompts[choice - 1]
                break
            else:
                print("Invalid number. Please try again.")
        except ValueError:
    ...
    # Load the actual prompt text from file
    try:
        prompt_text = load_prompt_text(selected_prompt['filepath'])
    except FileNotFoundError as e:
        print(f"ERROR: {e}")
        return

    # Create prompt_config in the format expected by query_llm
    prompt_config = {
        "prompt_text": prompt_text,
        "system_prompt": "",  # New system doesn't use system prompts in templates
    }

    print("\n--- READY TO SEND ---")
    print(f"Configuration: {chosen_config_name}")
    print(f"Prompt ID: {selected_prompt['run_id'][:8]} | Product: {selected_prompt['product_id']}")
    print(f"Template: {Path(selected_prompt['template']).stem} | Trap: {selected_prompt['trap_flag']}")
    ...
    data_to_save = {
        ...
        "prompt_id": selected_prompt['run_id'],  # Use run_id as prompt_id
        "prompt_text": prompt_text,
        "system_prompt": "",
        ...
        # Store additional metadata from new system
        "product_id": selected_prompt['product_id'],
        "template": selected_prompt['template'],
        "trap_flag": selected_prompt['trap_flag'],
    }
