#!/usr/bin/env python3
"""
Claude Post-Processing for RoBERTa Violations

Reads violations from glass_box_audit.py results and runs Claude analysis
...
Usage:
    python analysis/claude_postprocess_violations.py --input results/final_audit_results.csv
"""
...
def review_violation(
    ...
    """
    Use Claude to review a single violation flagged by RoBERTa.
    ...
    try:
        ...
        return {
            'verdict': 'ERROR',
            'severity': 'NONE',
            'reasoning': f'Error during Claude review: {str(e)}',
...
def main():
    parser = argparse.ArgumentParser(
        description='Claude post-processing for RoBERTa violations'
    )
    parser.add_argument(
        '--input',
        type=str,
    ...
    parser.add_argument(
        '--output',
        type=str,
        default='results/claude_filtered_violations.csv',
        help='Path to output CSV with Claude filtering'
    )
    parser.add_argument(
        '--limit',
        type=int,
    ...
    print("="*70)
    print("CLAUDE POST-PROCESSING FOR ROBERTA VIOLATIONS")
    print("="*70)
    print(f"\nInput: {input_path}")
    ...
    for i, (run_id, violations) in enumerate(materials_to_process.items(), 1):
        ...
        try:
            ...
            # Review each violation with Claude
            for v in violations:
                ...
                results.append({
                    ...
                    'Claude_Verdict': review.get('verdict', 'ERROR'),
                    'Claude_Severity': review.get('severity', 'NONE'),
                    'Claude_Reasoning': review.get('reasoning', ''),
                    'Recommended_Action': review.get('recommended_action', 'REVIEW'),
                    'Suggested_Fix': review.get('suggested_fix', ''),
                    'Claude_Confidence': f"{review.get('confidence', 0.0):.4f}"
                })
