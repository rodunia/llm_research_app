#!/usr/bin/env python3
"""
Claude Direct Compliance Audit
Analyzes all marketing materials directly with Claude Sonnet 4 (no NLI pre-filtering).
"""
...
# Output paths
CHECKPOINT_FILE = RESULTS_DIR / "claude_audit_checkpoint.jsonl"
FINAL_RESULTS_CSV = RESULTS_DIR / "claude_compliance_audit.csv"
SUMMARY_FILE = RESULTS_DIR / "claude_audit_summary.txt"
...
def analyze_material(material_text: str, product_id: str, run_id: str) -> Dict:
    """
    Analyze a marketing material for compliance violations using Claude.
...
def audit_all_materials(resume: bool = True, limit: int = None):
    ...
    """
    print("=" * 80)
    print("CLAUDE DIRECT COMPLIANCE AUDIT")
    print("=" * 80)
    print()
    ...
    print(f"✓ Estimated cost: ~${len(experiments) * 0.04:.2f} (${0.04}/run)")
    print(f"✓ Estimated time: ~{len(experiments) * 12 / 60:.1f} minutes")
    print()
...
def generate_summary(results: List[Dict]):
    ...
    summary = f"""
================================================================================
CLAUDE COMPLIANCE AUDIT - SUMMARY
================================================================================
...
def main():
    parser = argparse.ArgumentParser(description='Claude direct compliance audit')
